...
class ModelToolExportImport extends Model {
	...
	protected function validateCategories( &$reader ) {
		...
		// Opencart versions from 2.0 onwards also have category_description.meta_title
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."category_description` LIKE 'meta_h1'";
		$query = $this->db->query( $sql );
		...
		if ($this->use_table_seo_url) {
			if ($exist_meta_title) {
				if ($exist_meta_h1) {
					...
					( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "description", "meta_title", "meta_description", "meta_keywords", "store_ids", "layout", "status", "meta_h1" );
				} else {
					$expected_heading = array
				...
				if ($exist_meta_h1) {
					$expected_heading = array
					( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "description", "meta_description", "meta_keywords", "store_ids", "layout", "status", "meta_h1" );
				} else {
					$expected_heading = array
			...
			if ($exist_meta_title) {
				if ($exist_meta_h1) {
					$expected_heading = array
					( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_title", "meta_description", "meta_keywords", "store_ids", "layout", "status", "meta_h1" );
				} else {
					$expected_heading = array
				...
				if ($exist_meta_h1) {
					$expected_heading = array
					( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "seo_keyword", "description", "meta_description", "meta_keywords", "store_ids", "layout", "status", "meta_h1" );
				} else {
					$expected_heading = array
		...
		if ($exist_meta_title) {
			if ($exist_meta_h1) {
				$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_keywords", "meta_h1" );
			} else {
				$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_keywords" );
			}
		} else {
			if ($exist_meta_h1) {
				$expected_multilingual = array( "name", "description", "meta_description", "meta_keywords", "meta_h1" );
			} else {
...
//		$col -= 1; // we use 1-based, PHPExcel uses 0-based column index, PhpSpreadsheet now uses 1-based column index
	...
	protected function validateProducts( &$reader ) {
		...
		if ($exist_meta_title) {
			if ($exist_meta_h1) {
				$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_keywords", "tags", "meta_h1" );
			} else {
				$expected_multilingual = array( "name", "description", "meta_title", "meta_description", "meta_keywords", "tags" );
			}
		} else {
			if ($exist_meta_h1) {
				$expected_multilingual = array( "name", "description", "meta_description", "meta_keywords", "tags", "meta_h1" );
			} else {
	...
	protected function uploadCategories( &$reader, $incremental, &$available_category_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$this->storeCategoryIntoDatabase( $category, $languages, $exist_meta_title, $exist_meta_h1, $layout_ids, $available_store_ids, $url_alias_ids );
		}
	...
	protected function storeCategoryIntoDatabase( &$category, &$languages, $exist_meta_title, $exist_meta_h1, &$layout_ids, &$available_store_ids, &$url_alias_ids ) {
		...
		foreach ($languages as $language) {
			...
			if ($exist_meta_h1) {
				$meta_h1 = isset($meta_h1s[$language_code]) ? $this->db->escape($meta_h1s[$language_code]) : '';
			}
			...
			if ($exist_meta_title) {
				if ($exist_meta_h1) {
					$sql  = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `meta_h1`) VALUES ";
					$sql .= "( $category_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$meta_h1' );";
				} else {
				...
				if ($exist_meta_h1) {
					$sql  = "INSERT INTO `".DB_PREFIX."category_description` (`category_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `meta_h1`) VALUES ";
					$sql .= "( $category_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword', '$meta_h1' );";
				} else {
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$this->storeProductIntoDatabase( $product, $languages, $product_fields, $exist_table_product_tag, $exist_meta_title, $exist_meta_h1, $layout_ids, $available_store_ids, $manufacturers, $weight_class_ids, $length_class_ids, $url_alias_ids );
		}
	}
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, $exist_meta_h1, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		foreach ($languages as $language) {
			...
			if ($exist_table_product_tag) {
				if ($exist_meta_title) {
					if ($exist_meta_h1) {
						$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `meta_h1`) VALUES ";
						$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$meta_h1' );";
					} else {
					...
					if ($exist_meta_h1) {
						$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `meta_h1`) VALUES ";
						$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword', '$meta_h1' );";
					} else {
				...
				if ($exist_meta_title) {
					if ($exist_meta_h1) {
						$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `meta_h1`, `tag`) VALUES ";
						$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$meta_h1', '$tag' );";
					} else {
					...
					if ($exist_meta_h1) {
						$sql  = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `meta_h1`, `tag`) VALUES ";
						$sql .= "( $product_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword', '$meta_h1', '$tag' );";
					} else {
	...
	protected function populateCategoriesWorksheet( &$worksheet, &$languages, &$box_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null ) {
		...
		if ($exist_meta_h1) {
			foreach ($languages as $language) {
				$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('meta_h1'),20)+1);
			}
		}
		foreach ($languages as $language) {
		...
		$categories = $this->getCategories( $languages, $exist_meta_title, $exist_meta_h1, $exist_seo_url_table, $offset, $rows, $min_id, $max_id );
	...
	protected function getCategories( &$languages, $exist_meta_title, $exist_meta_h1, $exist_seo_url_table, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		if ($exist_seo_url_table) {
		...
		foreach ($languages as $language) {
			...
			foreach ($results->rows as $key=>$row) {
				if (isset($category_descriptions[$language_code][$key])) {
					...
					if ($exist_meta_h1) {
						$results->rows[$key]['meta_h1'][$language_code] = $category_descriptions[$language_code][$key]['meta_h1'];
					}
					...
					if ($exist_meta_h1) {
						$results->rows[$key]['meta_h1'][$language_code] = '';
					}
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		if ($exist_meta_h1) {
			foreach ($languages as $language) {
				$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('meta_h1')+4,20)+1);
			}
		}
		foreach ($languages as $language) {
		...
		$products = $this->getProducts( $languages, $default_language_id, $product_fields, $exist_meta_title, $exist_meta_h1, $exist_seo_url_table, $offset, $rows, $min_id, $max_id );
		$len = count($products);
	...
	protected function getProducts( &$languages, $default_language_id, $product_fields, $exist_meta_title, $exist_meta_h1, $exist_seo_url_table, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		$sql  = "SELECT ";
		$sql .= "  p.product_id,";
		...
		foreach ($languages as $language) {
			...
			foreach ($results->rows as $key=>$row) {
				if (isset($product_descriptions[$language_code][$key])) {
					...
					if ($exist_meta_h1) {
						$results->rows[$key]['meta_h1'][$language_code] = $product_descriptions[$language_code][$key]['meta_h1'];
					}
