	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			if ($result['manager_id'] != 0) {
				...
				$manager = $this->model_user_user->getUserByManager($result['manager_id']);
			} else {
				$manager = $this->language->get('entry_nothing_manager');
			}
		...
		$data['sort_manager'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.manager_id' . $url, true);
	...
	protected function validateForm() {
		...
		foreach ($custom_fields as $custom_field) {
			if (($custom_field['location'] == 'account') && $custom_field['required'] && empty($this->request->post['custom_field'][$custom_field['custom_field_id']])) {
				$field_error = $this->model_customer_custom_field->getError($custom_field['custom_field_id']);
				if ($field_error != "")
					$this->error['custom_field'][$custom_field['custom_field_id']] = $field_error;
				else
...
//		}
	...
	protected function getForm() {
		...
		if (isset($this->error['whosign'])) {
			$data['error_whosign'] = $this->error['whosign'];
		} else {
			$data['error_whosign'] = '';
		}
		...
		if (isset($this->request->post['manager_id'])) {
			...
			$data['manager_id'] = $customer_info['manager_id'];
		} else {
			$data['manager_id'] = 0;
		}
		...
		if (isset($this->request->post['whosign'])) {
			$data['whosign'] = $this->request->post['whosign'];
		} elseif (!empty($customer_info)) {
			$data['whosign'] = $customer_info['whosign'];
		} else {
			$data['whosign'] = '';
		}
		...
		$data['customer_managers'] = $this->model_user_user_group->getUsersByGroup(15);

        $data['customer_managers'] = array_merge($data['customer_managers'], $this->model_user_user_group->getUsersByGroup(16));
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email']) || isset($this->request->get['filter_company']) || isset($this->request->get['filter_customer_id'])) {
			...
			if (isset($this->request->get['filter_company']) && strlen($this->request->get['filter_company']) > 2) {
