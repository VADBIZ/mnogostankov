...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'manager_id'	=> $result['order_manager_id'],
				'manager_name'  => $this->model_user_user->getUserByManager($result['order_manager_id']),
				...
				'contract'		=> $this->url->link('sale/contract', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . '&show=true' . $url, true),
				'contract_download'	=> $this->url->link('sale/contract', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . '&show=false' . $url, true),
			);
		}
		...
		$data['sort_manager'] = $this->url->link('sale/order', 'user_token=' . $this->session->data['user_token'] . '&sort=o.order_manager_id' . $url, true);
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$data['custom_fields'] = $order_info['custom_field'];
			$data['pact_type_id'] = $order_info['pact_type_id'];
			$data['pact_sign'] = $order_info['pact_sign'];
			$data['term_pay'] = $order_info['term_pay'];
			$data['time_delivery'] = $order_info['time_delivery'];
			$data['sublist'] = $order_info['sublist'];
			$data['foundoc'] = $order_info['foundoc'];
			$data['garantime'] = $order_info['garantime'];
			$data['comwork'] = $order_info['comwork'];
			$data['delivery'] = $order_info['delivery'];

			//вычисляем менеджера
			$manager = false;
			if ($this->user->getGroupId() == 15) {
				$manager = true;
				$data['order_manager_id'] = $this->user->getId();
			} else {
				$data['order_manager_id'] = $order_info['order_manager_id'];
			}
		...
		$companies = array();
		$this->load->model('customer/customer');
		$companiesQuery = $this->model_customer_customer->getCustomers();
		foreach($companiesQuery as $companyQuery) {
			if ($companyQuery['company'] == "") {
				$companyName = $this->language->get('entry_company_empty');
			} else {
				$companyName = $companyQuery['company'];
			}
			$data['companies'][] = array(
				'customer_id'	=> $companyQuery['customer_id'],
				'name'			=> $companyName." - ".$companyQuery['firstname']." ".$companyQuery['lastname']
			);
		}
		...
		foreach ($custom_fields as $custom_field) {
			if (isset($order_custom_fields[$custom_field['custom_field_id']])) {
				$custom_value = $order_custom_fields[$custom_field['custom_field_id']];
			} else {
				$custom_value = "";
			}
			$data['custom_fields'][] = array(
		...
		$resultPacts = $this->model_sale_pact->getPacts();
		foreach ($resultPacts as $result) {
			$data['pacts'][] = array(
		...
		if ($files) {
			foreach ($files as $file) {
				$shipping_method = basename($file, '.php');
				if ($this->config->get('shipping_' . $shipping_method . '_status')) {
					$this->load->language('extension/shipping/' . $shipping_method, 'extension');
					$data['shipping_methods'][] = array(
		...
		if ($files) {
			foreach ($files as $file) {
				...
				if ($this->config->get('payment_' . $payment_method . '_status')) {
					$this->load->language('extension/payment/' . $payment_method, 'extension');
					$data['payment_methods'][] = array(
						'code'		 => $payment_method,
						'name'       => $this->language->get('extension')->get('heading_title')
					);
		...
		$data['order_managers'] = $this->model_user_user_group->getUsersByGroup(15);

		// Руководители продаж
		$data['order_managers'] = array_merge($data['order_managers'], $this->model_user_user_group->getUsersByGroup(16));
		...
		$data['button_createContract'] = $this->url->link('sale/contract', 'user_token=' . $this->session->data['user_token'] . "&order_id=".$data['order_id'] . '&show=false', true);
	...
	public function uploadsign() {
		...
		if (!$json) {
			...
			move_uploaded_file($this->request->files['file']['tmp_name'], DIR_PACT . $file);

			$json['filename'] = $file;
			$json['mask'] = $filename;
			$json['link'] = DIR_PACT_LINK . $file;

			$json['success'] = $this->language->get('success_upload');
			$json['pact_sign_change'] = $this->language->get('pact_sign_change');
		}
