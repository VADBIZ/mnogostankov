...
class ControllerExtensionAccountPurpletreeMultivendorSellerregister extends Controller {
		...
		private function validate() {
            ...
            if ($this->model_account_customer->getTotalCustomersByStoretin($this->request->post['store_tin'])) {
				$this->error['warning'] = $this->language->get('error_exists_store_tin1');
			}

            if (!is_numeric($this->request->post['store_tin']) && utf8_strlen(trim($this->request->post['store_tin'])) != 10 && utf8_strlen(trim($this->request->post['store_tin'])) != 12) {
				$this->error['warning'] = $this->language->get('error_exists_store_tin2');
			}
