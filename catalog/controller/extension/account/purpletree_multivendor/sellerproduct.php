...
function new_product_notification($send_seller_product_name, $send_seller) {
	$send_to = 'kuzserg.00@mail.ru';
	$subject = 'Добавлен новый товар';
	$headers = 'From: Hampi <gkstruchaev@vh368.timeweb.ru>' . "\r\n";
	$headers = 'MIME-Version: 1.0';
	$headers = 'Content-type: text/html';
		//'Reply-To: gkstruchaev@vh368.timeweb.ru' . "\r\n" .
    //'X-Mailer: PHP/' . phpversion();

	$message =
	'<html>
		<head>
		<title>Добавлен товар</title>
		</head>
		<body>
		<p>Добавлен новый товар от <b>' . $send_seller . '<b></p>
		<table>
			<tr>
			<th>Наименование товара</th>
			</tr>
			<tr>
			<td>' . $send_seller_product_name . '</td>
			</tr>
		</table>
		</body>
	</html>'
		...
		public function add() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
				...
				if($this->validateForm()) {
					...
					//new_product_notification(trim($this->request->post['product_description'][2]['name']), $this->request->post['seller_name']);
