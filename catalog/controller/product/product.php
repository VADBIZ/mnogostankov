...
//        		}
		...
		if ($product_info) {
            ...
            if ($product_info['image']) {
                $data['video_image'] = str_replace('watch?v=', 'embed/', $product_info['video_image']);
            } else {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				if ($product_info['price'] != "0.0000") {
					$price_usd = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
					$price_currency = $this->currency->get_currency('USD', 4);
					$price_rub = number_format((float)$price_usd * $price_currency, 0, ',', ' ');
			...
			if (!is_null($product_info['special']) && (float)$product_info['special'] >= 0) {
				...
				$tax_price1 = (float)$product_info['special'];
				$tax_price2 = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
				$tax_price = $tax_price2 - $tax_price1;
			} else {
				$data['special'] = false;
				$tax_price1 = (float)$product_info['price'];
				$tax_price2 = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
				$tax_price = $tax_price2 - $tax_price1;
			}
