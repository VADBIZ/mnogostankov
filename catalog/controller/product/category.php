		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if (!is_null($result['special']) && (float)$result['special'] >= 0) {
					...
					$tax_price1 = (float)$result['special'];
					$tax_price2 = $this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax'));
					$tax_price = $tax_price2 - $tax_price1;
				} else {
					$special = false;
					$tax_price1 = (float)$result['price'];
					$tax_price2 = $this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax'));
					$tax_price = $tax_price2 - $tax_price1;
				}
